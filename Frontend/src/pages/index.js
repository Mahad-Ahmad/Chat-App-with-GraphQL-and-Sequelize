...
export default function Home() {
  ...
  // const { messageLoading } = useLazyQuery(GET_M, {
  //   onCompleted: (data) => {
  //     const { getUsers } = data;
  //     setUsers(getUsers);
  //   },
            ...
            <div class="flex flex-col space-y-1 mt-4 -mx-2 h-full overflow-y-auto">
              {users.map((el,i) => {
                return (
                  <button
                    key={i}
                    onClick={() => onChatClick(el.name)}
                    class="flex flex-row items-center hover:bg-gray-100 rounded-xl p-2"
                  >
                    <img
                      src={el.imageUrl}
                      class="h-12 w-12 bg-indigo-200 rounded-full object-cover"
                    />
                    <div class="ml-3 mb-2 text-sm text-center font-semibold">
                      {el.name}
                    </div>
                  </button>
                );
